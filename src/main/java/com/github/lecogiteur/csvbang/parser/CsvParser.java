...
public class CsvParser<T> {
		...
		do{
			...
			if (!isContentNull){
				for (int contentOffset=0; contentOffset < content.length;){
					...
					final CsvGrammarAction<?> a = generateAction2(newAction, content.length > contentOffset?content.length-contentOffset:0);
			...
			if (isLastDatagram && !CsvGrammarActionType.END.equals(action.getType())){
				//it's the last datagramm for this file
				action = generateAction2(CsvGrammarActionType.END, 0);
			...
			if (!stack.isEmpty()){
				if (backupFileOffset != fileOffset && CsvGrammarActionType.UNDEFINED.equals(stack.getLast().getType())){
					//perhaps we can analyze another part of the stack, we try
					final UndefinedGrammarAction a = (UndefinedGrammarAction)stack.removeLast();
					content = a.execute();
					fileOffset = a.getStartOffset();
					...
					stackByOffset.put(generateID(fileID, stack.getFirst().getStartOffset(), fileOffset), stack);
				}
			}
		}while (mustContinue);
		return listOfBeans;
	...
	private void tryToGroupAndClearStack(final Collection<T> csvBeans, final Deque<CsvGrammarAction<?>> stack, 
		...
		if (stack.getLast().isLastAction()){
			//terminate action
			CsvGrammarAction<?> a = stack.pollLast();
			while (!stack.isEmpty() && stack.getLast().add(a)){
				a = stack.pollLast();
			}
			stack.addLast(a);
		}
	...
	private CsvGrammarAction<?> initAction(final Deque<CsvGrammarAction<?>> stack, final long fileOffset, final int contentLength){
		...
		final CsvGrammarAction<?> action = generateAction2(fileOffset == 0?CsvGrammarActionType.START:CsvGrammarActionType.UNDEFINED, contentLength);
		action.setStartOffset(fileOffset);
		return action;
	}
	 ...
	 * @since 1.0.0
	 */
	private CsvGrammarAction<?> generateAction2(final CsvGrammarActionType action, final int contentLength){
		switch (action) {
		case FIELD:
